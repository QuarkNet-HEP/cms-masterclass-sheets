...
def get_sheet_id_by_title(service, spreadsheet_id: str, title: str) -> str:

    # Get sheet metadata
    metadata = (
        service.spreadsheets().get(spreadsheetId=spreadsheet_id).execute(num_retries=5)
    )
...
def resize_sheet(service, spreadsheet_id: str, tab_id: str, nrows: int):
        ...
        "requests": [
            {
                "updateSheetProperties": {
                    "properties": {
                        "sheetId": tab_id,
                        "gridProperties": {"rowCount": nrows, "columnCount": 27},
                    },
    ...
    service.spreadsheets().batchUpdate(
        spreadsheetId=spreadsheet_id, body=resize_request
...
def append_rows(service, spreadsheet_id: str, tab_id: str, n_rows: int):
    """
...
def add_dataset_numbers(
    service, sheet_name: str, spreadsheet_id: str, row_number: int, dataset_number: int
):
    updates = []
...
def make_vstack_formula(
    tab_names: List[str], col: str, start_row: str, end_row: Optional[str] = None
) -> str:
    if end_row is None:
...
def main(name, tabs):
    ...
    about = drive_service.about().get(fields="user").execute(num_retries=5)
            ...
            [
                {
                    ...
                    "values": [
                        [
                            f"=C{totals_row} + 2*(E{totals_row}+I{totals_row}) + 4*(G{totals_row})"
                        ]
                    ],
                },
                {
                    ...
                    "values": [
                        [
                            f"=D{totals_row} + 2*(F{totals_row}+I{totals_row}) + 4*(H{totals_row})"
                        ]
                    ],
                },
            ...
            [
                {
                    "copyPaste": {
                        ...
                        "destination": {
                            ...
                            - 1,  # exclusive; fill up to row totals_row-1
                            "startColumnIndex": 24,
                            "endColumnIndex": 25,
                        },
                        "pasteType": "PASTE_NORMAL",
